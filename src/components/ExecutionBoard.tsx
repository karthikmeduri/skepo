import { AlertTriangle, Bot as BotIcon, Check, CheckCircle2, ChevronRight, Circle, Clock3, Network, Pause, Play, RefreshCw, RotateCcw, ShieldCheck, Square, XCircle, Zap } from 'lucide-react'
import { useMemo, useState } from 'react'
import ReactMarkdown from 'react-markdown'
import remarkGfm from 'remark-gfm'
import type { Bot, WorkflowBudget, WorkflowManifest, WorkflowStartRequest, WorkflowTask } from '../types'
...
const defaultBudget: WorkflowBudget = { maxTasks: 8, maxParallel: 2, maxRetries: 1, maxTotalTokens: 24_000, timeoutMinutes: 30, approvePlan: true }
...
export function ExecutionBoard(props: Props) {
  ...
  return <main className="page-view runs-page">
    ...
    {composerOpen && <RunComposer bots={props.bots} workspacePath={props.workspacePath} ollamaUrl={props.ollamaUrl} busy={props.busy} onClose={() => setComposerOpen(false)} onStart={async request => { await props.onStart(request); setComposerOpen(false) }} />}
  </main>
}

function RunComposer({ bots, workspacePath, ollamaUrl, busy, onClose, onStart }: { bots: Bot[]; workspacePath: string; ollamaUrl: string; busy: boolean; onClose(): void; onStart(request: WorkflowStartRequest): Promise<void> }) {
